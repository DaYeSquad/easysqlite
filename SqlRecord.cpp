...
string Record::toSqlInsertIfNotExists(string tableName, string key, string value) {
  string s = "INSERT INTO " + tableName + " ";
  s += "(" + _fields->toString() + ")";
  s += " SELECT ";
  s += toSql();
  s += " WHERE NOT EXISTS(SELECT 1 FROM " + tableName + " WHERE " + key + "='" + value + "')";
  
  return s;
}

string Record::toSqlInsertOrReplace(string tableName)
{
  string s = "insert or replace into " + tableName + " ";
...
string Record::toSqlUpdate(string tableName, string where_condition) {
  string s = "update " + tableName + " set ";
