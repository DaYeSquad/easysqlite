...
bool Table::open(string whereCondition, string sortBy, bool asc) {
  string asc_str = asc ? "ASC" : "DESC";
  
  const string queryStr = "select * from " + _tableName
  + (whereCondition.empty() ? "" : " where " + whereCondition)
  + (sortBy.empty() ? "" : " order by " + sortBy + " " + asc_str);
...
////////////////////////////////////////////////////////////////////////////////
// SQLTable, LCC_EXTENSION:

bool Table::addRecordIfNotExists(Record* record, string key, string value) {
  if (record)
  {
    const string sql = record->toSqlInsertIfNotExists(name(), key, value);
...
bool Table::addOrReplaceRecord(Record *record)
{
  if (record)
  {
    const string sql = record->toSqlInsertOrReplace(name());
...
bool Table::updateRecord(Record* record, string where_condition) {
  if (record)
  {
    const string sql = record->toSqlUpdate(name(), where_condition);
